...
/*eslint no-useless-escape: "off", quotes: "off"*/
...
dust.helpers['if'] = function(chunk, context, bodies, params) {
  ...
  if (params) {
    ...
    if (result) {
      if (body) {
        ...
        console.warn('Missing body block in the if helper!');
        return chunk;
      }
    }
    if (skip) {
  ...
  // no condition
    ...
    console.warn('No condition given in the if helper!');
  }
  return chunk;
};
...
function tokenize(input) {
  ...
  while (pos < input.length) {
    ...
    if (cc === SPACE || cc === TAB) {
      ...
      cc === MINUS ||
      (cc === DOT &&
        input.charCodeAt(pos + 1) >= ZERO &&
        input.charCodeAt(pos + 1) <= NINE)
    ) {
      // type = INT;
      if (
        (cc === ZERO && input.charCodeAt(pos + 1) === x) ||
        input.charCodeAt(pos + 1) === X
      ) {
        // hex can not contain dots or exponents
        while (++pos < input.length && regexHex.test(input[pos + 1])) {
          console.warn('hex can not contain dots or exponents');
        }
      ...
      if (result) {
        ...
        if (c.length) {
          ...
          pushToken(
            tokens,
            ERR,
            'Invalid Expression near ' + input[pos] + input[pos + 1]
          );
          return tokens;
        }
      }
  ...
  if (
    lastTok.type === OP &&
    lastTok.value !== RPARENOP &&
    lastTok.value !== RBRACKOP
  ) {
    pushToken(
      tokens,
      ERR,
      'Invalid expression, ended with an operator:' + input
    );
  }
...
function collectExponent(input, pos) {
  ...
  if (
    input.charCodeAt(++pos + 1) === PLUS ||
    input.charCodeAt(pos + 1) === MINUS
  ) {
    pos += 1;
  }
  pos = collectDigits(input, pos);
...
function evaluate(toks, expr, ctx) {
  ...
  while (i < toks.length) {
    ...
    i++;
    if (tok.type === OP) {
      evalOperator(opstk, opndstk, tok);
